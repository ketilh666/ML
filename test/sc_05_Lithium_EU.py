...
fname_smp  = 'Train_Test_Samples_and_Features_EU.xlsx'
fname_temp = 'Train_Test_Samples_and_Features_Temp_EU.xlsx'
...
#--------------------------------------------
#  Read map data from pickle file
#--------------------------------------------

with open(pkl + 'Grids_10km_EU.pkl', 'rb') as fid:
...
# Which test to run:
krun, version = 2, ''   # version = empty or a, b, c etc
print(f'### Run {krun}{version}')
split, var = 'Random',  'MLE'
if   krun ==  0: # No good
     ind_feat_use = [ii for ii in range(len(feat_list_all))]
elif krun ==  1: #  
     ind_feat_use = [3,4,5,7,8,9,10,11,13]
elif krun ==  2: #  
     ind_feat_use = [3,4,5,7,8,9,10,11]

# Out put files
crun = 'run' + str(krun) + version
out_file_model = '_EU_Li_classification_model.pkl'
out_file_pred  = '_EU_Li_Predicted_Maps.pkl' 
out_file_excel = '_EU_Li_Predicted_Maps.xlsx' 
...
fig.savefig(png + 'All_Maps_EU.png')    
...
fig.savefig(png+ f'Supervised_EU_{crun}.png')
...
fig.savefig(png+ f'Train&test_EU.png')
